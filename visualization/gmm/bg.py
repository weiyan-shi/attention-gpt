...
# Label 0: 55 points (35.26%)
# Label 1: 23 points (14.74%)
# Label 2: 23 points (14.74%)
# Label 3: 55 points (35.26%)
# Label distribution for TD in file 33:
# Label 0: 46 points (41.82%)
# Label 1: 15 points (13.64%)
# Label 2: 42 points (38.18%)
# Label 3: 7 points (6.36%)
...
OutputDir = "C:\\Users\\86178\\Desktop\\attention-gpt\\visualization\\gmm\\res"

files = ["33"]

n_components = 4  # Number of components (clusters) for GMM
...
for file in files:
    ...
    try:
        ...
        # Apply GMM clustering
        gmm_labels_TD = perform_gmm_clustering(points_TD, n_components)

        # Print label distribution for TD
        print(f"Label distribution for TD in file {FileName}:")
        print_label_distribution(gmm_labels_TD)

        # Create an empty image for the convex hulls
        overlay_TD = np.zeros_like(Img_TD)

        # Overlay clustering results with convex hulls
        for label in set(gmm_labels_TD):
            label_points = points_TD[gmm_labels_TD == label]
        ...
        output_path_ASD = os.path.join(OutputDir, f"{FileName}_ASD_gmm.png")
        output_path_TD = os.path.join(OutputDir, f"{FileName}_TD_gmm.png")
