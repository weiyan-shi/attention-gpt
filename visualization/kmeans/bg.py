...
# Label 1: 24 points (15.38%)
# Label 2: 22 points (14.10%)
# Label 3: 60 points (38.46%)
# Label distribution for TD in file 33:
# Label 0: 42 points (38.18%)
# Label 1: 13 points (11.82%)
# Label 2: 40 points (36.36%)
# Label 3: 15 points (13.64%)
...
OutputDir = "C:\\Users\\86178\\Desktop\\attention-gpt\\visualization\\kmeans"
...
for file in files:
    ...
    try:
        ...
        # Apply KMeans clustering
        kmeans_labels_TD = perform_kmeans_clustering(points_TD, k)

        # Print label distribution for TD
        print(f"Label distribution for TD in file {FileName}:")
        print_label_distribution(kmeans_labels_TD)

        # Create an empty image for the convex hulls
        overlay_TD = np.zeros_like(Img_TD)

        # Overlay clustering results with convex hulls
        for label in set(kmeans_labels_TD):
            label_points = points_TD[kmeans_labels_TD == label]
        ...
        output_path_ASD = os.path.join(OutputDir, f"{FileName}_ASD_kmeans.png")
        output_path_TD = os.path.join(OutputDir, f"{FileName}_TD_kmeans.png")
