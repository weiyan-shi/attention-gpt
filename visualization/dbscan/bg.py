...
# Label 0: 49 points (32.89%)
# Label 1: 35 points (23.49%)
# Label distribution for TD in file 3:
# Label -1: 43 points (40.19%)
# Label 0: 64 points (59.81%)
...
OutputDir = "C:\\Users\\86178\\Desktop\\attention-gpt\\visualization\\dbscan"
...
for file in files:
    ...
    try:
        ...
        # Apply DBSCAN clustering
        dbscan_labels_TD = perform_dbscan_clustering(points_TD, eps, min_samples)

        # Print label distribution for TD
        print(f"Label distribution for TD in file {FileName}:")
        print_label_distribution(dbscan_labels_TD)

        # Create an empty image for the convex hulls
        overlay_TD = np.zeros_like(Img_TD)

        # Overlay clustering results with convex hulls
        for label in set(dbscan_labels_TD):
            if label == -1:
                continue  # Skip noise

            label_points = points_TD[dbscan_labels_TD == label]
        ...
        output_path_ASD = os.path.join(OutputDir, f"{FileName}_ASD_dbscan.png")
        output_path_TD = os.path.join(OutputDir, f"{FileName}_TD_dbscan.png")
