...
# Label -1: 113 points (72.44%)
# Label 0: 16 points (10.26%)
# Label 1: 27 points (17.31%)
# Label distribution for TD in file 33:
# Label -1: 28 points (25.45%)
# Label 0: 38 points (34.55%)
# Label 1: 44 points (40.00%)
...
OutputDir = "C:\\Users\\86178\\Desktop\\attention-gpt\\visualization\\optics\\res"
...
def perform_optics_clustering(data, min_samples, max_eps):
    optics = OPTICS(min_samples=min_samples, max_eps=max_eps).fit(data)
...
for file in files:
    ...
    try:
        ...
        # Apply OPTICS clustering
        optics_labels_TD = perform_optics_clustering(points_TD, min_samples, max_eps)

        # Print label distribution for TD
        print(f"Label distribution for TD in file {FileName}:")
        print_label_distribution(optics_labels_TD)

        # Create an empty image for the convex hulls
        overlay_TD = np.zeros_like(Img_TD)

        # Overlay clustering results with convex hulls
        for label in set(optics_labels_TD):
            if label == -1:
                continue  # Skip noise

            label_points = points_TD[optics_labels_TD == label]
        ...
        output_path_ASD = os.path.join(OutputDir, f"{FileName}_ASD_optics.png")
        output_path_TD = os.path.join(OutputDir, f"{FileName}_TD_optics.png")
