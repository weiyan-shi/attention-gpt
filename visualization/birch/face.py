...
# Label 0: 94 points (63.09%)
# Label 1: 55 points (36.91%)
# Label distribution for TD in file 3:
# Label 0: 31 points (28.97%)
# Label 1: 76 points (71.03%)
...
OutputDir = "C:\\Users\\86178\\Desktop\\attention-gpt\\visualization\\birch\\res"

files = ["3"]

n_clusters = 2  # Number of clusters for BIRCH
...
def perform_birch_clustering(data, n_clusters):
...
for file in files:
    ...
    try:
                ...
                color = (
                    ...
                    if label == 1
                        ...
                        if label == 0
                            ...
                            colors["Pink"]
                            if label == 2
                            else colors["Orange"] if label == 3 else colors["White"]
                        )
                    )
                )
        ...
        overlay_ASD_transparent = cv2.addWeighted(
            overlay_ASD, 0.8, np.zeros_like(overlay_ASD), 0.2, 0
        )

        # Combine the darkened image and the transparent hulls
        combined_ASD = cv2.addWeighted(
            Img_ASD_darkened, 1.0, overlay_ASD_transparent, 1.0, 0
        )
        ...
        # Apply BIRCH clustering
        birch_labels_TD = perform_birch_clustering(points_TD, n_clusters)

        # Print label distribution for TD
        print(f"Label distribution for TD in file {FileName}:")
        print_label_distribution(birch_labels_TD)

        # Create an empty image for the convex hulls
        overlay_TD = np.zeros_like(Img_TD)

        # Overlay clustering results with convex hulls
        for label in set(birch_labels_TD):
            label_points = points_TD[birch_labels_TD == label]
        ...
        # Apply transparency to the filled hulls
        overlay_TD_transparent = cv2.addWeighted(
            overlay_TD, 0.8, np.zeros_like(overlay_TD), 0.2, 0
        )

        # Combine the darkened image and the transparent hulls
        combined_TD = cv2.addWeighted(
            Img_TD_darkened, 1.0, overlay_TD_transparent, 1.0, 0
        )

        # Save the output images
        output_path_ASD = os.path.join(OutputDir, f"{FileName}_ASD_birch.png")
        output_path_TD = os.path.join(OutputDir, f"{FileName}_TD_birch.png")
