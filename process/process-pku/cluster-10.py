...
asd_file1 = 'C:\\Users\\86178\\Desktop\\attention-gpt\\dataset\\pku\\HFA.csv'
asd_file2 = 'C:\\Users\\86178\\Desktop\\attention-gpt\\dataset\\pku\\LFA.csv'
td_file = 'C:\\Users\\86178\\Desktop\\attention-gpt\\dataset\\pku\\TD.csv'
OutputCSV = 'C:\\Users\\86178\\Desktop\\attention-gpt\\dataset\\pku\\ClusteringResults_10.csv'
...
def evaluate_clustering(data, labels):
    if len(np.unique(labels)) > 1:
        silhouette_avg = SC(data, labels)
        ch_score = CH(data, labels)
        db_score = DB(data, labels)
        csl_score = CSL(data, labels)
        di_score = DI(data, labels)
        db_star_score = DB_star(data, labels)
        gd33_score = GD33(data, labels)
        pb_score = PB(data, labels)
        pbm_score = PBM(data, labels)
        str_score = STR(data, labels)
        return {
            'SC': silhouette_avg,
            'CH': ch_score,
            'DB': db_score,
...
def process_combination(row, df):
    ...
    movie = row['TrialNumber']
    patient_type = row['Class']
    
    data_subset = df[(df['Name'] == participant) & (df['TrialNumber'] == movie) & (df['Class'] == patient_type)]
    
    X = data_subset['LeftEyeX'].values
    Y = data_subset['LeftEyeY'].values
    ...
    try:
        ...
        print(f"Processed Participant: {participant}, Stimulus: {movie}, Type: {patient_type}")
...
asd_df1 = pd.read_csv(asd_file1)
asd_df2 = pd.read_csv(asd_file2)
td_df = pd.read_csv(td_file)

df = pd.concat([asd_df1, asd_df2, td_df], ignore_index=True)

unique_combinations = df[['Name', 'TrialNumber', 'Class']].drop_duplicates()
