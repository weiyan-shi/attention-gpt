...
df1 = pd.read_csv(file1).dropna()
df2 = pd.read_csv(file2).dropna()
df3 = pd.read_csv(file3).dropna()
...
train_loader_A = prepare_data(X1_train, y1_train)
train_loader_B = prepare_data(X2_train, y2_train)
train_loader_C = prepare_data(X3_train, y3_train)

test_loader_A = prepare_data(X1_test, y1_test)
test_loader_B = prepare_data(X2_test, y2_test)
test_loader_C = prepare_data(X3_test, y3_test)

train_loaders = [train_loader_A, train_loader_B, train_loader_C]
test_loaders = [test_loader_A, test_loader_B, test_loader_C]
...
# 联邦学习训练过程
num_epochs = 10
local_epochs = 10  # 每轮联邦学习中的本地训练轮数
